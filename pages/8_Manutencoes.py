...
def abrir_ordem_servico(aparelho_id, fornecedor, defeito):
    ...
    try:
        ...
        ultimo_colaborador = cursor.execute("""
            ...
            WHERE aparelho_id = ? AND colaborador_id IS NOT NULL 
...
def carregar_historico_manutencoes(status_filter=None, colaborador_filter=None):
    """Carrega o histórico completo de manutenções com filtros."""
    conn = get_db_connection()
    query = """
        SELECT 
    ...
    if status_filter and status_filter != "Todos":
        where_clauses.append("m.status_manutencao = ?")
        params.append(status_filter)
    
    if colaborador_filter and colaborador_filter != "Todos":
        where_clauses.append("c.nome_completo = ?")
...
with tab3:
    ...
    conn = get_db_connection()
    status_manutencao_options = ["Todos"] + [row['status_manutencao'] for row in conn.execute("SELECT DISTINCT status_manutencao FROM manutencoes").fetchall()]
    colaboradores_options = ["Todos"] + [row['nome_completo'] for row in conn.execute("SELECT nome_completo FROM colaboradores ORDER BY nome_completo").fetchall()]
    conn.close()
